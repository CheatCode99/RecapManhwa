import {
  listEdgeVoices,
  generateSingleEdgeAudio,
  generateBatchEdgeAudio
} from "../../services/edge-tts-service.js";
    ...
    <main class="tts-page">
      <section class="tts-shell">
        <header class="tts-header">
          <h1 class="tts-title">Welcome to Edge-TTS</h1>
        </header>
        ...
        <section class="tts-section">
          ...
          <div class="tts-slider-row">
            <label class="tts-slider-label" for="tts-rate">Speech Rate</label>
            <input
              type="range"
              min="-50"
              max="100"
          ...
          <div class="tts-slider-row">
            <label class="tts-slider-label" for="tts-pitch">Pitch</label>
            <input
              type="range"
              min="-50"
              max="50"
          ...
          <div class="tts-slider-row">
            <label class="tts-slider-label" for="tts-volume">Volume</label>
            <input
              type="range"
              min="-50"
              max="50"
...
async function getPreviewOutputPath(format) {
  ...
  return await path.join(cacheDir, `edge_preview.${format}`);
}
...
function bindTtsEvents() {
  ...
  document.querySelector("#tts-language")?.addEventListener("change", loadVoices);
}
...
function bindTtsUi() {
  ...
  const rateSlider = document.querySelector("#tts-rate");
  const rateValue = document.querySelector("#tts-rate-value");
  const pitchSlider = document.querySelector("#tts-pitch");
  const pitchValue = document.querySelector("#tts-pitch-value");
  const volumeSlider = document.querySelector("#tts-volume");
  const volumeValue = document.querySelector("#tts-volume-value");

  if (rateSlider && rateValue) {
    rateValue.textContent = formatPercent(rateSlider.value);
  ...
  if (pitchSlider && pitchValue) {
    pitchValue.textContent = formatHz(pitchSlider.value);
  ...
  if (volumeSlider && volumeValue) {
    volumeValue.textContent = formatPercent(volumeSlider.value);
...
function getSelectedLanguage() {
  return document.querySelector("#tts-language")?.value || "en";
}
...
async function loadVoices() {
  ...
  try {
    ...
    const result = await listEdgeVoices({ language: getSelectedLanguage() });
    ...
    const stefanVoice = result.voices.find((voice) =>
      voice.toLowerCase().includes("steffan")
    );

    if (stefanVoice) {
      voiceSelect.value = stefanVoice;
    ...
    console.error("Failed to load Edge voices:", error);
    if (status) {
...
async function handleSingleGenerate() {
  ...
  const rate = formatPercent(document.querySelector("#tts-rate")?.value || "0");
  const pitch = formatHz(document.querySelector("#tts-pitch")?.value || "0");
  const volume = formatPercent(document.querySelector("#tts-volume")?.value || "0");
  const removeSilence = document.querySelector("#tts-remove-silence")?.checked;
  const format = document.querySelector("#tts-format")?.value || "mp3";
  ...
  try {
    ...
    setGenerationProgress(20, "Preparing generation...");

    const progressTimer = startIndeterminateProgress();
    const result = await generateSingleEdgeAudio({
    ...
    setGenerationProgress(100, "Audio generated successfully.");
    ttsState.lastGeneratedAudioPath = result.output_path;
    loadGeneratedAudioIntoPlayer(result.output_path);
  } catch (error) {
    console.error("Failed to generate Edge audio:", error);
...
async function handleBatchGenerate() {
  ...
  try {
    ...
    const result = await generateBatchEdgeAudio({
      language: getSelectedLanguage(),
      inputTxt: inputPath,
      outputDir,
      voice,
